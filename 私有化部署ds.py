...
def get_completion(prompts, model, tokenizer=None, max_tokens=8192, temperature=0.6, top_p=0.95, max_model_len=2048):
    ...
    llm = LLM(model=model, tokenizer=tokenizer, max_model_len=max_model_len, trust_remote_code=True,
              dtype="float16")  # V100 不支持bfloat16 类型，需要设置为 float16
...
if __name__ == "__main__":
    # 初始化 vLLM 推理引擎
    model = '/root/autodl-tmp/models/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B'  # 指定模型路径
    ...
    outputs = get_completion(text, model, tokenizer=tokenizer, max_tokens=8192, temperature=0.6, top_p=0.95,
                             max_model_len=2048)  # 思考需要输出更多的 Token 数，max_tokens 设为 8K，根据 DeepSeek 官方的建议，temperature应在 0.5-0.7，推荐 0.6
